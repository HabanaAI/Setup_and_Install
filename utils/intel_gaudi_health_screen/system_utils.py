...
class KubeUtils(SystemUtils):
    ...
    def check_screen_complete(self, current_run_status, health_report, level, round=0):
        if level == 1:
            metadata_app = "ighs"
        ...
        cmd     = f"kubectl get pods -n {self.namespace} -l app={metadata_app} -o=custom-columns='NAME:.metadata.name,STATUS:.status.phase' --no-headers"
        output  = run_cmd(cmd).strip()
        pods    = output.split("\n")
