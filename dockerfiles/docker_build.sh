...
: "${1?"Usage: $0 mode [tensorflow,pytorch] os [ubuntu18.04,ubuntu20.04,amzn2] tf_version [2.4.1, 2.5.0]"}"
: "${2?"Usage: $0 mode [tensorflow,pytorch] os [ubuntu18.04,ubuntu20.04,amzn2] tf_version [2.4.1, 2.5.0]"}"
: "${2?"Usage: $0 mode [tensorflow,pytorch] os [ubuntu18.04,ubuntu20.04,amzn2] tf_version [2.4.1, 2.5.0]"}"

VERSION="${CUSTOM_VERSION:-0.15.4}"
REVISION="${CUSTOM_REVISION:-75}"
MODE="$1"
OS="$2"
TF_VERSION="$3"
ARTIFACTORY_URL="${CUSTOM_ARTIFACTORY_URL:-vault.habana.ai/artifactory}"
...
function buildDocker {
    ...
    case $MODE in
        ...
        if [ -z "$5" ]; then
            ...
            echo "Usage: $0 mode [tensorflow,pytorch] os [ubuntu18.04,ubuntu20.04,amzn2] tf_version [2.4.1, 2.5.0]"
            exit 1
        fi
